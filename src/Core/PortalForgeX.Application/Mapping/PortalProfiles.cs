...
public class PortalProfiles : Profile
{
    public PortalProfiles()
    {
        ...
        CreateMap<TenantSettings, TenantViewModel>()
        ...
        CreateMap<ApplicationUser, TenantViewModel>()
            .ForMember(x => x.ManagerId, opt => opt.MapFrom(x => x.Id))
            .ForMember(x => x.ManagerName, opt => opt.MapFrom(x => x.UserName));
